...
int main(void)
{
 ...
 unsigned char *text1, temp = 0;
 GPIO_Configuration();
 GPIO_WriteBit(GPIOB,GPIO_Pin_1,1);
 GPIO_WriteBit(GPIOB,GPIO_Pin_15,1);
 ADC1_Init();
 USART_setup();
text1 = "Send ADC data to PC via Serial\n\r\0";
 USART1_putS(text1);
 DelayMC(1000);
 while(1)
 {
 ADCRead_value = readADC1(0);
 temp = (int) (ADCRead_value /1000)%10;
 USART1_sendC(digits[temp]);
 temp = (int) (ADCRead_value /100)%10;
 USART1_sendC(digits[temp]);
 temp = (int) (ADCRead_value /10)%10;
 USART1_sendC(digits[temp]);
 temp = (int) (ADCRead_value)%10;
 USART1_sendC(digits[temp]);
 USART1_sendC(',');
