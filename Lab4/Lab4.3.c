...
int main(void)
{
 unsigned int i = 0, ADCRead_value = 0;
 unsigned int highV =0 , lowV =0;
 ...
 write_I2C1( PCF8591_7BIT_ADDR <<1 |0, 0x00);
 while(1)
 {

  i--;
  ADCRead_value = read_I2C1( PCF8591_7BIT_ADDR <<1 |0);
  highV = (int)(ADCRead_value*3.3)/255;
  lowV = (int)((ADCRead_value*100/255) * 3.3) % 100;
  itoa(highV, sbuf, 10);
  USART1_putS(sbuf);
  USART1_putS(".");
  itoa(lowV, sbuf, 10);
